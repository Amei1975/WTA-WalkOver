...
descriptive_wta <- descrTable(WalkOver ~ tourney_category + year + surface + round_level + winner_hand + loser_hand + winner_age + loser_age + dif_age + mean_age + winner_rank + loser_rank + dif_rank, chisq.test.perm = TRUE ,data = wta_ba, method=2, byrow = TRUE)

compare_wta <- compareGroups(WalkOver ~ tourney_category + year + surface + round_level + winner_hand + loser_hand + winner_age + loser_age + dif_age + mean_age + winner_rank + loser_rank + dif_rank, chisq.test.perm = TRUE ,data = wta_ba, method=2, byrow = TRUE)
