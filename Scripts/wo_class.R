                    ...
                    title_wo="")
{
    ...
    for(i in year_i:year_f)
    {
        ...
        base_year <- nrow(x[x$year==i,])
        if (n_year[i-(year_i-1)] != 0 | base_year != 0)
        {
        if (distribution=="Poisson")
            {
                ic_year <- pois.exact(n_year[i-(year_i-1)],base_year)
                incidence[i-(year_i-1)] <- ic_year$rate*1000
            }
            if (distribution=="Binomial")
            {
                ic_year <- binom.exact(n_year[i-(year_i-1)],base_year)
                incidence[i-(year_i-1)] <- ic_year$proportion*1000
            }
            lower[i-(year_i-1)] <- ic_year$lower*1000
            upper[i-(year_i-1)] <- ic_year$upper*1000
            var_year[i-(year_i-1)] <- (n_year[i-(year_i-1)]/(base_year^2))*1000*1000
    ...
    ci_up <- ic$upper*1000
    dd <- data.frame(year=year,n=n_year,incidence=incidence,lower=lower,upper=upper,var_year=var_year)
                    ...
                    title_wo="Incidence of Walkovers in WTA tennis matches by year")
{
    ...
    result <- ggplot(dd,aes(x=dd$year,y=dd$incidence)) + 
                        ...
                        labs(x="Year", 
                             y="WalkOver Incidence Rate per 1,000 matches",
                             title=title_wo) +
